...
def show_attention_summary_popup(panel_root, summary: dict):
    ...
    win.title("Riepilogo sessione — Attenzione")
    ...
    tk.Label(header, text="📊  Riepilogo sessione attenzione",
    ...
    kpis = [
        ("Media attenzione", f"{avg:.1f}%",  attention_color(avg)),
        ("Picco",            f"{peak:.0f}%", GREEN),
        ("Minimo",           f"{low:.0f}%",  attention_color(low)),
        ("Campioni totali",  str(checks),    TEAL),
        ("Durata stimata",   duration_str,   TEXT_MUTED),
    ]
    ...
    tk.Label(bar_frame, text="Livello medio attenzione",
    ...
    tk.Label(bar_frame,
             text="  ≥70% Ottimo     40–70% Sufficiente     <40% Basso",
             font=("Arial", 8), bg=DARK_BG, fg=TEXT_MUTED).pack(anchor="w")

    # ── Grafico andamento ─────────────────────────────────────────────────────
    tk.Label(win, text="Andamento soglia attenzione nel tempo",
    ...
    def draw_chart(canvas):
        ...
        # Linee soglia colorate tratteggiate
    ...
    for color, lbl in [(TEAL, "Andamento"), (GREEN, "≥70%"), (ORANGE, "40–70%"), (RED, "<40%")]:
